...
import com.example.movies.net.API_QUERY
...
class MoviePagingSource(private val service: Webservice) : PagingSource<Int, Movie>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Movie> {
        val position = params.key ?: INIT_PAGE_INDEX
        Timber.d("hc, initkey: $position")
        return try {
            val response = service.getMovieList(API_KEY, API_QUERY, position)
